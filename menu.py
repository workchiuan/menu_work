...
import uuid
import io # 新增 io 庫，用於處理圖片的二進位資料
    ...
    if st.button("🚀 確認發起團購", type="primary"):
        ...
        if not vendor_name:
            ...
            new_group = {
                ...
                "menu_image_bytes": image_bytes # 儲存圖片資料
            }
        ...
        if group:
            ...
            if group.get('menu_image_bytes'):
                with st.expander("🖼️ 點此查看原始菜單圖片 (參考用)", expanded=False):
                    # 將儲存的 bytes 轉為 Streamlit 可讀取的 buffer
            ...
            if time_left.total_seconds() <= 0:
                ...
                with st.form(key=f"form_{group['id']}"):
                    ...
                    with col_u2:
                        ...
                        selected_item_str = st.selectbox("選擇餐點", menu_options)
                    ...
                    if group['category'] == "飲料":
                        ...
                        with c_bev1:
                            ...
                            sugar_choice = st.selectbox("甜度", sugar_opts)
                        with c_bev2:
                            ice_opts = ["(請選擇)", "正常冰", "少冰", "微冰", "去冰", "完全去冰", "溫", "熱"]
                            ice_choice = st.selectbox("冰塊", ice_opts)

                    col_q1, col_q2 = st.columns(2)
                    with col_q1:
                        quantity = st.number_input("數量", min_value=1, value=1)
                    with col_q2:
                        note = st.text_input("其他備註 (例如：加珍珠)")
